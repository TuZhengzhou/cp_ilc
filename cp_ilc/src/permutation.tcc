...
pp_perm<FieldT>::pp_perm(const size_t mu, const size_t n, const size_t col_num, const row_vector_matrix<FieldT>& A, const row_vector_matrix<FieldT>& B, \
    ...
    this->V_PI_ = apply_permutation<FieldT, 2>(this->V_, PI);
...
bool pp_perm<FieldT>::prove() {
    libff::enter_block("pp_perm<FieldT>::prove()");
    ...
    libff::leave_block("pp_perm<FieldT>::prove()");
    return true;
}

template <typename FieldT>
bool pp_perm<FieldT>::verify(const bool output) const {
    libff::enter_block("pp_perm<FieldT>::verify()");
    ...
    libff::leave_block("pp_perm<FieldT>::verify()");
    if (check_result == true) {
        if (output) {
            libff::print_indent(); printf("pp_perm<FieldT>::verify() \033[32mpass\033[37m\n\n");
        }
        return true;
    }
    if (output) {
        libff::print_indent(); printf("pp_perm<FieldT>::verify() \033[31mfail\033[37m\n\n");
    }
    return false;
}
...
bool permutation_test() {
    ...
    pp_perm<FieldT> perm = pp_perm<FieldT>(mu, n, col_num, A, A, PI);
    perm.is_satisfy();
