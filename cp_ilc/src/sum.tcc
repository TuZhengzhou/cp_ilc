...
bool pp_sum<FieldT>::verify(const row_vector<FieldT> expected_result, const bool output) const {
    ...
    libff::enter_block("pp_sum<FieldT>::verify()");
    ...
    libff::leave_block("pp_sum<FieldT>::verify()");

    /* 查询结果 与 期望结果对比 */
    if (open != expected_result) {
        if (output) {
            libff::print_indent(); printf("pp_sum<FieldT>::verify() \033[31mfail\033[37m\n\n");
        }
        return false;
    }
    if (output) {
        libff::print_indent(); printf("pp_sum<FieldT>::verify() \033[32mpass\033[37m\n\n");
    }
    return true;
}
