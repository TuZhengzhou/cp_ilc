    ...
    mu_(mu), m_(std::pow(2, mu)), n_(n), row_num_(std::pow(2, mu) * n), col_num_(col_num)
{
    ...
    // libff::print_indent(); printf("mu = \033[32m%ld\033[37m, m = \033[32m%ld\033[37m, n = \033[32m%ld\033[37m, col_num = \033[32m%ld\033[37m\n", this->mu_, this->m_, this->n_, this->col_num_);
...
bool pp_prod<FieldT>::prove() {
    libff::enter_block("pp_prod<FieldT>::prove()");
    // 随机生成 a0, b0, c0
    ...
    libff::leave_block("pp_prod<FieldT>::prove()");
    return true;
}

template<typename FieldT>
bool pp_prod<FieldT>::verify(const bool output) const {
    libff::enter_block("pp_prod<FieldT>::verify()");
    /* 选取随机挑战 */
    FieldT x = FieldT::random_element();
    ...
    libff::leave_block("pp_prod<FieldT>::verify()");
    if (a_dot * b_dot != c_dot) {
        if (output) {
            libff::print_indent(); printf("pp_prod<FieldT>::verify \033[31mfail\033[37m\n\n");
        }
        return false;
    }
    if (output) {
        libff::print_indent(); printf("pp_prod<FieldT>::verify \033[32mpass\033[37m\n\n");
    }
    return true;
}
