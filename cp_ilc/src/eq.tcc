...
bool pp_eq<FieldT>::verify(const row_vector_matrix<FieldT>& pub_matrix, const bool output) const {
    ...
    libff::enter_block("pp_eq<FieldT>::verify()");
    ...
    libff::leave_block("pp_eq<FieldT>::verify()");
    /* 比较 open 结果与 期望计算结果 */
    if (open != expected_result) {
        if (output) {
            libff::print_indent(); printf("pp_eq<FieldT>::verify() \033[31mfail\033[37m\n\n");
        }
        return false;
    }
    if (output) {
        libff::print_indent(); printf("pp_eq<FieldT>::verify() \033[32mpass\033[37m\n\n");
    }
    return true;
}
