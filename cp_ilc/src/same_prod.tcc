...
bool pp_same_prod<FieldT>::prove() {
    libff::enter_block("pp_same_prod<FieldT>::prove");

    this->pp_prod_A_.prove();
    this->pp_prod_B_.prove();
    this->pp_shift_AB_.prove();

    libff::leave_block("pp_same_prod<FieldT>::prove");
    return true;
};

template<typename FieldT>
bool pp_same_prod<FieldT>::verify(const bool output) const {
    libff::enter_block("pp_same_prod<FieldT>::verify()");
    ...
    libff::leave_block("pp_same_prod<FieldT>::verify()");
    if ( result ) {
        if (output) {
            libff::print_indent(); printf("pp_same_prod<FieldT>::verify() \033[032mpass\033[037m\n\n");
        }
        return true;
    }
    if (output) {
        libff::print_indent(); printf("pp_same_prod<FieldT>::verify() \033[031mfail\033[037m\n\n");
    }
    return false;
};
